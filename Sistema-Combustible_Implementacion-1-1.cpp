...
void mostrarMenu() {
    ...
    std::cout << "3. Verificar fuga de combustible\n";
...
int main() {
    ...
    srand(time(0)); // Incializar generador numero randomico
    ...
    do {
        ...
        switch (opcion) {
            case 1: {
                ...
                std::cin >> gerente;
                std::cout << "Ingrese la región de la estación: ";
                std::cin >> region;

                EstacionServicio* estacion = new EstacionServicio(nombre, gerente, region);
                redNacional.agregarEstacion(estacion);
                break;
            }
            case 2: {
                std::cout << "Ingrese el código de la estación: ";
                std::cin >> codigoEstacion;
                redNacional.simularVenta(codigoEstacion);
                break;
            }
            case 3: {
                std::cout << "Ingrese el código de la estación: ";
                std::cin >> codigoEstacion;
                if (redNacional.verificarFugaCombustible(codigoEstacion)) {
                    std::cout << "Fuga de combustible detectada.\n";
                } else {
                    std::cout << "No hay fuga de combustible.\n";
                }
                break;
            }
            case 4: {
